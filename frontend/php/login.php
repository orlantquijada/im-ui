...
if (isset($_POST["submit"])) {
    if (!empty($_POST['Username']) && !empty($_POST['Password'])) {
        ...
        $sql = "SELECT count(*) as count FROM employee WHERE username= '$Username'";
        $result = mysqli_query($con, $sql);
        $row = mysqli_fetch_array($result);

        $sql = "SELECT count(*) as count as count FROM employee WHERE password= '$Password'";
        $resultpass = mysqli_query($con, $sql);
        $rowpass = mysqli_fetch_array($resultpass);

        if ($row[0] == 1) {
            $sql = "SELECT count(*) as count FROM employee WHERE password= '$Password'";
            $result = mysqli_query($con, $sql);
            $row = mysqli_fetch_array($result);
            if ($row[0] == 1) {
                session_start();
